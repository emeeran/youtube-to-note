...
export class YouTubeTranscriptService {
    ...
    private async downloadCaptionTrack(
        ...
        try {
            transcript = this.parseXMLTranscript(xml, track.languageCode || 'en', track.kind === 'asr');
        } catch (error) {
            logger.warn('Transcript: caption payload was not parseable XML', 'Transcript', {
                videoId,
                error: error instanceof Error ? error.message : String(error),
            });
            return {
                ok: false,
                reason: 'unknown',
                message: 'Caption payload was malformed (an error page or truncated response), not a caption document.',
            };
        }
    ...
    private parseXMLTranscript(xmlText: string, language: string, autoGenerated: boolean): TranscriptResult {
        ...
        // DOMParser never throws — a malformed payload (HTML error page,
        // consent wall, truncated body) comes back as a <parsererror>
        // document. Callers must not mistake that for a caption-less video.
        if (xmlDoc.getElementsByTagName('parsererror').length > 0) {
            throw new Error('caption payload is not well-formed XML');
        }
