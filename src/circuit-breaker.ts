...
export class CircuitBreaker {
    ...
    private changeState(newState: CircuitState): void {
        ...
        if (oldState !== newState) {
            ...
            console.info(`CircuitBreaker '${this.name}' state changed from ${oldState} to ${newState}`);
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    console.warn(`CircuitBreaker '${this.name}' attempt ${attempt + 1} failed, retrying in ${delay}ms:`, error);
    ...
    import(stateJson: string): void {
        try {
            ...
            console.info(`CircuitBreaker '${this.name}' state imported successfully`);
        } catch (error) {
            console.error(`Failed to import CircuitBreaker '${this.name}' state:`, error);
        }
    }
