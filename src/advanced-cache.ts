            ...
            keys.forEach(key => {
                if (key.startsWith('cache_')) {
                    try {
                        ...
                        console.error('Failed to load persistent cache item:', error);
                        this.persistentCache!.removeItem(key);
                    }
                }
            });
        } catch (error) {
            console.error('Failed to load persistent cache data:', error);
        }
    }
        ...
        const promises = preloadData.map(async ({ key, loader, priority = 1.0 }) => {
            if (!this.has(key)) {
                try {
                    ...
                    console.warn(`Failed to warmup cache item ${key}:`, error);
                }
            }
        });
