...
var YouTubeSettingsTab = class extends import_obsidian6.PluginSettingTab {
  ...
  */
  createAPISettings() {
    ...
    const geminiSetting = new import_obsidian6.Setting(containerEl).setName("Gemini API Key").setDesc("Your Google Gemini API key for content processing (password field - secured)").addText((text) => {
    ...
    const groqSetting = new import_obsidian6.Setting(containerEl).setName("Groq API Key").setDesc("Your Groq API key for fallback processing (password field - secured)").addText((text) => {
    ...
    new import_obsidian6.Setting(testSection).setName("Test API Connection").setDesc("Verify your API keys are valid").addButton((btn) => btn.setButtonText("Test Keys").onClick(async () => {
  ...
  createSecuritySettings() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Use Environment Variables").setDesc("Load API keys from environment variables instead of storing them in configuration").addToggle((toggle) => toggle.setValue(this.settings.useEnvironmentVariables || false).onChange(async (value) => {
      await this.updateSetting("useEnvironmentVariables", value);
      this.display();
    }));
    if (this.settings.useEnvironmentVariables) {
      new import_obsidian6.Setting(containerEl).setName("Environment Variable Prefix").setDesc("Prefix for environment variable names (e.g., YTC_GEMINI_API_KEY)").addText((text) => text.setPlaceholder("YTC").setValue(this.settings.environmentPrefix || "YTC").onChange(async (value) => {
      ...
      new import_obsidian6.Setting(envSection).setName("Copy Template").setDesc("Copy the environment variables template to clipboard").addButton((button) => button.setButtonText("Copy to Clipboard").onClick(() => {
  ...
  createFileSettings() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Output Path").setDesc("Directory path where processed videos will be saved (relative to vault root)").addText((text) => text.setPlaceholder(MESSAGES.PLACEHOLDERS.OUTPUT_PATH).setValue(this.settings.outputPath).onChange(async (value) => {
