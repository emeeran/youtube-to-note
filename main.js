...
</div>`}))(ve||(ve={}));var Et={fast:`Analyze this YouTube video efficiently:
Title: {{TITLE}}
URL: {{URL}}
...
- Use markdown headers (##, ###) for structure, not numbered lines
- Numbered lists are for content only (steps, items), not for line references
- Process multimodally (visual/audio) for complete understanding including on-screen text and non-verbal cues`},wt={"concise-summary":`# Concise Summary
...
### Conclusion
Final thoughts and key takeaways`},Z=class{createAnalysisPrompt(t){var p;let{videoData:e,videoUrl:r,format:i="step-by-step-tutorial",transcript:s,performanceMode:n="balanced"}=t,o=(p=b.extractVideoId(r))!=null?p:Ge.VIDEO_ID,a=G.AI_PROVIDER,l=G.AI_MODEL,c=this.buildBaseContent(e,r,s,n);return this.buildFullPrompt(c,e,r,o,i,a,l)}buildBaseContent(t,e,r,i="balanced"){let s=Et[i],n=this.buildTranscriptSection(r);return this.replacePlaceholders(s,{[G.TITLE]:t.title,[G.URL]:e,[G.DESCRIPTION]:t.description,[G.TRANSCRIPT_SECTION]:n})}buildTranscriptSection(t){return t!=null&&t.trim()?`
VIDEO CONTENT/TRANSCRIPT:
${t.length>ge.MAX_TRANSCRIPT_LENGTH?`${t.slice(0,ge.MAX_TRANSCRIPT_LENGTH)}... [transcript truncated]`:t}`:""}buildFullPrompt(t,e,r,i,s,n,o,a){let l=ve.frontmatter(e.title,r,i,s,n,o),c=ve.videoIframe(i,e.title),p=`---
...
${d}`}buildFormatTemplate(t,e){return wt[t]}replacePlaceholders(t,e){let r=t;for(let[i,s]of Object.entries(e)){let n=i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");r=r.replace(new RegExp(n,"g"),s)}return r}processAIResponse(t,e,r,i,s,n){if(!t)return t;let o=e!=null?e:Ge.PROVIDER,a=r!=null?r:Ge.MODEL,l=this.replacePlaceholders(t,{[G.AI_PROVIDER]:o,[G.AI_MODEL]:a});return l=this.ensureFrontMatterValue(l,"ai_provider",o),l=this.ensureFrontMatterValue(l,"ai_model",a),n&&(l=this.appendResourcesSection(l,n,o,a)),l}appendResourcesSection(t,e,r,i){let s=new Date().toISOString().split("T")[0],n=`
...
`).filter(l=>l.trim().length>0),o=(i=n[0])!=null?i:"";if(n.length===1&&b.isValidYouTubeUrl(o)&&e.length<200){let l=Date.now()-t.stat.ctime,c=t.path.includes(this.settings.outputPath);if(l<this.config.tempFileAgeThreshold&&!c)return h.debug("File identified as temp file via content analysis","UrlHandler",{filePath:t.path,fileAge:l,isInOutputPath:c,contentLength:e.length}),!0}return!1}catch(s){return h.error("Error checking if file is temp file","UrlHandler",{filePath:t.path,error:s instanceof Error?s.message:String(s)}),!1}}extractUrl(t){var e;try{if(t!=null&&t.includes(this.config.noteMarker)){let o=t.replace(this.config.noteMarker,"").trim();return b.isValidYouTubeUrl(o)?o:null}let r=t.trim(),i=/https?:\/\/(?:www\.)?youtube\.com\/watch\?v=[A-Za-z0-9_-]+(?:&[^/\s]*)?/i,s=/https?:\/\/(?:www\.)?youtu\.be\/[A-Za-z0-9_-]+(?:\?[^/\s]*)?/i,n=(e=r.match(i))!=null?e:r.match(s);if(n!=null&&n[0]){let o=n[0].trim();return b.isValidYouTubeUrl(o)?o:null}return null}catch(r){return h.error("Error extracting URL from content","UrlHandler",{error:r instanceof Error?r.message:String(r),contentLength:t.length}),null}}handleUrlSafely(t){if(h.info("URL detected, processing safely","UrlHandler",{url:t.url,source:t.source,filePath:t.filePath}),t.file&&t.content&&!this.isTempFile(t.file,t.content)){h.warn("URL rejected - not in temp file","UrlHandler",{url:t.url,filePath:t.filePath});return}if(this.handledTempFiles.has(t.url)){h.debug("URL already handled, skipping","UrlHandler",{url:t.url});return}this.pendingUrls.has(t.url)&&(h.debug("Cancelling pending handler for URL","UrlHandler",{url:t.url}),clearTimeout(this.pendingUrls.get(t.url))),this.handledTempFiles.add(t.url),t.filePath&&this.handledTempFiles.add(t.filePath);let e=setTimeout(()=>{h.info("Processing URL after debounce","UrlHandler",{url:t.url}),this.onUrlDetected(t),this.pendingUrls.delete(t.url),this.cleanupHandledFiles()},this.config.urlHandlerDelay);this.pendingUrls.set(t.url,e)}cleanupHandledFiles(){if(this.handledTempFiles.size>this.config.maxHandledFiles){let t=Array.from(this.handledTempFiles);this.handledTempFiles.clear(),t.slice(-Math.floor(this.config.maxHandledFiles/2)).forEach(e=>{this.handledTempFiles.add(e)}),h.debug("Cleaned up handled temp files","UrlHandler",{oldSize:t.length,newSize:this.handledTempFiles.size})}}async handleFileCreate(t){try{if(!(t!=null&&t.path)||!t.stat)return;let e=await this.app.vault.read(t);if(!this.isTempFile(t,e)){h.debug("Ignoring non-temp file in create handler","UrlHandler",{filePath:t.path});return}let r=this.extractUrl(e);if(!r){h.debug("No URL extracted from temp file","UrlHandler",{filePath:t.path});return}let i={url:r,source:"create",filePath:t.path,file:t,content:e};h.info("CREATE EVENT - detected temp note","UrlHandler",{...i}),this.handleUrlSafely(i)}catch(e){h.error("Error handling file create","UrlHandler",{filePath:t.path,error:e instanceof Error?e.message:String(e)})}}async handleActiveLeafChange(){try{let t=this.app.workspace.getActiveFile();if(!t||!("stat"in t)||!("basename"in t)||!("extension"in t)||this.handledTempFiles.has(t.path))return;let e=await this.app.vault.read(t);if(!this.isTempFile(t,e)){h.debug("Ignoring non-temp file in active leaf handler","UrlHandler",{filePath:t.path});return}let r=this.extractUrl(e);if(!r){h.debug("No URL extracted from temp file in active leaf","UrlHandler",{filePath:t.path});return}let i={url:r,source:"active-leaf",filePath:t.path,file:t,content:e};h.info("ACTIVE-LEAF-CHANGE EVENT - detected temp note","UrlHandler",{...i}),this.handleUrlSafely(i)}catch(t){h.error("Error handling active leaf change","UrlHandler",{error:t instanceof Error?t.message:String(t)})}}handleProtocol(t){var e,r,i;try{let s=(i=(r=(e=t.url)!=null?e:t.content)!=null?r:t.path)!=null?i:"";if(s&&b.isValidYouTubeUrl(s)){let n={url:s,source:"protocol"};h.info("Protocol handler received valid URL","UrlHandler",{...n}),setTimeout(()=>{this.onUrlDetected(n)},200)}else h.debug("Protocol handler received no valid URL","UrlHandler",{params:t})}catch(s){h.error("Error in protocol handler","UrlHandler",{params:t,error:s instanceof Error?s.message:String(s)})}}async handleClipboardUrl(){var t,e;try{let r="";try{(t=navigator==null?void 0:navigator.clipboard)!=null&&t.readText&&(r=(e=await navigator.clipboard.readText())!=null?e:"")}catch(i){h.debug("Could not read clipboard","UrlHandler",{error:i instanceof Error?i.message:String(i)}),r=""}if(r&&b.isValidYouTubeUrl(r.trim())){let i={url:r.trim(),source:"clipboard"};h.info("URL found in clipboard","UrlHandler",{...i}),this.onUrlDetected(i);return}h.debug("No valid YouTube URL in clipboard","UrlHandler")}catch(r){h.error("Error handling clipboard URL","UrlHandler",{error:r instanceof Error?r.message:String(r)})}}clear(){this.pendingUrls.forEach(t=>clearTimeout(t)),this.pendingUrls.clear(),this.handledTempFiles.clear(),h.info("URL handler cleared","UrlHandler")}updateSettings(t){this.settings=t,h.debug("URL handler settings updated","UrlHandler")}};var _=require("obsidian");var m="ytc-settings",Ue=class extends _.PluginSettingTab{constructor(e,r){super(e,r.plugin);this.options=r;this.validationErrors=[];this.drawerStates=new Map;this.DRAWER_STATES_KEY="ytc-settings-drawer-states";this.providerStatuses=new Map;this.settings={...r.plugin.settings},this.secureConfig=new W(this.settings),this.loadDrawerStates()}display(){let{containerEl:e}=this;e.empty(),e.addClass(`${m}-container`),this.settings={...this.options.plugin.settings},this.secureConfig=new W(this.settings),this.injectStyles(),this.createHeader(),this.createSearchBar(),this.createProviderStatusDashboard(),this.createQuickActions(),this.createAPISection(),this.createAISection(),this.createOutputSection(),this.createAdvancedSection()}injectStyles(){if(document.getElementById(`${m}-styles`))return;let e=document.createElement("style");e.id=`${m}-styles`,e.textContent=`
        ...
        `;let g=!1,f=()=>{var Ze,et;let w=(Ze=this.selectedProvider)!=null?Ze:"Google Gemini",y=(et=this.selectedModel)!=null?et:"Default";c.textContent=`${w}`,c.title=`${w} \u2022 ${this.formatModelName(y)}`},E=()=>{g=!g,d.style.display=g?"block":"none",l.classList.toggle("active",g),l.style.borderColor=g?"var(--ytc-accent)":"var(--ytc-border)",p.style.transform=g?"rotate(180deg)":"rotate(0deg)"};l.addEventListener("click",E),l.addEventListener("keydown",w=>{(w.key==="Enter"||w.key===" ")&&(w.preventDefault(),E())});let P=d.createDiv(),A=P.createEl("label");A.textContent="AI PROVIDER",A.htmlFor="ytc-provider-select",A.style.cssText="font-size: 0.7rem; font-weight: 600; margin-bottom: 6px; color: var(--ytc-text-muted); letter-spacing: 0.05em; display: block;",this.providerSelect=P.createEl("select"),this.providerSelect.id="ytc-provider-select",[{value:"Google Gemini",text:"Google Gemini (Recommended)"},{value:"Groq",text:"Groq (Fastest)"},{value:"OpenRouter",text:"OpenRouter"},{value:"Hugging Face",text:"Hugging Face"},{value:"Ollama",text:"Ollama (Local)"},{value:"Ollama Cloud",text:"Ollama Cloud"}].forEach(w=>{if(!this.providerSelect)return;let y=this.providerSelect.createEl("option");y.value=w.value,y.textContent=w.text}),this.providerSelect.value=(ie=this.selectedProvider)!=null?ie:"Google Gemini";let K=d.createDiv();K.style.cssText="margin-top: 16px;";let q=K.createEl("label");q.textContent="MODEL",q.htmlFor="ytc-model-select",q.style.cssText="font-size: 0.7rem; font-weight: 600; color: var(--ytc-text-muted); letter-spacing: 0.05em; display: block; margin-bottom: 6px;";let C=K.createDiv();C.style.cssText="display: flex; gap: 8px; align-items: center;",this.modelSelect=C.createEl("select"),this.modelSelect.id="ytc-model-select",this.modelSelect.style.cssText=`
            flex: 1;
            width: 100%;
            padding: 10px 12px;
            border: 1px solid var(--background-modifier-border);
            ...
            height: 42px;
            cursor: pointer;
        `;let H=C.createEl("button",{cls:"ytc-icon-btn"});H.innerHTML="\u{1F504}",H.title="Refresh Models",H.setAttribute("aria-label","Refresh Models"),H.onclick=async w=>{w.stopPropagation(),H.innerHTML="\u23F3",await this.fetchModelsForCurrentProvider(),H.innerHTML="\u{1F504}",f()};let J=C.createEl("button",{cls:"ytc-icon-btn"});J.innerHTML="\u2B50",J.title="Save as Default Preference",J.setAttribute("aria-label","Save as Default Preference"),J.onclick=w=>{w.stopPropagation(),this.selectedModel&&(x.setPreference("preferredModel",this.selectedModel),new Y.Notice("\u2B50 Model saved as default!"))},this.providerSelect.addEventListener("change",async()=>{var w,y;this.selectedProvider=(y=(w=this.providerSelect)==null?void 0:w.value)!=null?y:"Google Gemini",await this.fetchModelsForCurrentProvider(),f()}),this.modelSelect.addEventListener("change",()=>{var w;this.selectedModel=(w=this.modelSelect)==null?void 0:w.value,f()}),f();let ue=this.updateModelDropdown.bind(this);this.updateModelDropdown=w=>{ue(w),f()}}createIconsRow(){}createVideoPreviewSection(e){this.videoPreviewContainer=e.createDiv(),this.videoPreviewContainer.style.cssText=`
            display: none;
            margin-top: 4px;
            padding: 6px;
            background: var(--background-secondary);
