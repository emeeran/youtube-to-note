...
`}};var F=require("obsidian"),d="ytc-settings",Ae=class extends F.PluginSettingTab{constructor(e,r){super(e,r.plugin);this.options=r;this.validationErrors=[];this.drawerStates=new Map;this.DRAWER_STATES_KEY="ytc-settings-drawer-states";this.providerStatuses=new Map;this.settings={...r.plugin.settings},this.secureConfig=new te(this.settings),this.loadDrawerStates()}display(){let{containerEl:e}=this;e.empty(),e.addClass(`${d}-container`),this.settings={...this.options.plugin.settings},this.secureConfig=new te(this.settings),this.injectStyles(),this.createHeader(),this.createSearchBar(),this.createProviderStatusDashboard(),this.createQuickActions(),this.createAPISection(),this.createAISection(),this.createOutputSection(),this.createAdvancedSection(),this.createHelpSection(),this.createKeyboardShortcutsInfo()}injectStyles(){if(document.getElementById(`${d}-styles`))return;let e=document.createElement("style");e.id=`${d}-styles`,e.textContent=`
            .${d}-container {
                max-width: 700px;
                margin: 0 auto;
            }
        ...
        `;let i=r.createDiv();i.style.flex="1";let s=i.createEl("label");s.textContent="OUTPUT FORMAT",s.htmlFor="ytc-format-select",s.style.cssText=`
            ...
            display: block;
        `,this.formatSelect=i.createEl("select"),this.formatSelect.id="ytc-format-select",[{value:"executive-summary",text:"\u{1F4CA} Executive Summary"},{value:"detailed-guide",text:"\u{1F4D8} Tutorial / Guide"},{value:"brief",text:"\u26A1 Brief Summary"},{value:"transcript",text:"\u{1F4DD} Transcript Note"},{value:"custom",text:"\u270F\uFE0F Custom Format"}].forEach(y=>{let M=this.formatSelect.createEl("option");M.value=y.value,M.textContent=y.text}),this.formatSelect.value=this.format,this.formatSelect.addEventListener("change",()=>{var y,M;this.format=(M=(y=this.formatSelect)==null?void 0:y.value)!=null?M:"executive-summary",this.toggleCustomPromptVisibility(),P.setPreference("lastFormat",this.format)});let a=r.createDiv();a.style.flex="1";let n=a.createEl("label");n.textContent="AI CONFIGURATION",n.style.cssText=`
            font-size: 0.7rem;
            font-weight: 600;
            color: var(--ytc-text-muted);
            margin-bottom: 6px;
        ...
        `,this.createCustomPromptSection(e);let g=!1,b=()=>{let y=this.selectedProvider||"Google Gemini",M=this.selectedModel||"Default";c.textContent=`${y}`,c.title=`${y} \u2022 ${this.formatModelName(M)}`},C=()=>{g=!g,m.style.display=g?"block":"none",l.classList.toggle("active",g),l.style.borderColor=g?"var(--ytc-accent)":"var(--ytc-border)",h.style.transform=g?"rotate(180deg)":"rotate(0deg)"};l.addEventListener("click",C),l.addEventListener("keydown",y=>{(y.key==="Enter"||y.key===" ")&&(y.preventDefault(),C())});let x=m.createDiv(),Q=x.createEl("label");Q.textContent="AI PROVIDER",Q.htmlFor="ytc-provider-select",Q.style.cssText="font-size: 0.7rem; font-weight: 600; margin-bottom: 6px; color: var(--ytc-text-muted); letter-spacing: 0.05em; display: block;",this.providerSelect=x.createEl("select"),this.providerSelect.id="ytc-provider-select",[{value:"Google Gemini",text:"Google Gemini (Recommended)"},{value:"Groq",text:"Groq (Fastest)"},{value:"OpenRouter",text:"OpenRouter"},{value:"Hugging Face",text:"Hugging Face"},{value:"Ollama",text:"Ollama (Local)"},{value:"Ollama Cloud",text:"Ollama Cloud"}].forEach(y=>{let M=this.providerSelect.createEl("option");M.value=y.value,M.textContent=y.text}),this.providerSelect.value=this.selectedProvider||"Google Gemini";let R=m.createDiv();R.style.cssText="margin-top: 16px;";let V=R.createEl("label");V.textContent="MODEL",V.htmlFor="ytc-model-select",V.style.cssText="font-size: 0.7rem; font-weight: 600; color: var(--ytc-text-muted); letter-spacing: 0.05em; display: block; margin-bottom: 6px;";let B=R.createDiv();B.style.cssText="display: flex; gap: 8px; align-items: center;",this.modelSelect=B.createEl("select"),this.modelSelect.id="ytc-model-select",this.modelSelect.style.cssText=`
            flex: 1;
            width: 100%;
            padding: 10px 12px;
            border: 1px solid var(--background-modifier-border);
            ...
            height: 42px;
            cursor: pointer;
        `;let O=B.createEl("button",{cls:"ytc-icon-btn"});O.innerHTML="\u{1F504}",O.title="Refresh Models",O.setAttribute("aria-label","Refresh Models"),O.onclick=async y=>{y.stopPropagation(),O.innerHTML="\u23F3",await this.fetchModelsForCurrentProvider(),O.innerHTML="\u{1F504}",b()};let K=B.createEl("button",{cls:"ytc-icon-btn"});K.innerHTML="\u2B50",K.title="Save as Default Preference",K.setAttribute("aria-label","Save as Default Preference"),K.onclick=y=>{y.stopPropagation(),this.selectedModel&&(P.setPreference("preferredModel",this.selectedModel),new G.Notice("\u2B50 Model saved as default!"))},this.providerSelect.addEventListener("change",async()=>{var y;this.selectedProvider=((y=this.providerSelect)==null?void 0:y.value)||"Google Gemini",await this.fetchModelsForCurrentProvider(),b()}),this.modelSelect.addEventListener("change",()=>{var y;this.selectedModel=(y=this.modelSelect)==null?void 0:y.value,b()}),b();let Re=this.updateModelDropdown.bind(this);this.updateModelDropdown=y=>{Re(y),b()}}createIconsRow(){}createVideoPreviewSection(e){this.videoPreviewContainer=e.createDiv(),this.videoPreviewContainer.style.cssText=`
            display: none;
            margin-top: 4px;
            padding: 6px;
            background: var(--background-secondary);
        ...
        `,this.fallbackToggle=a.createEl("input",{type:"checkbox"}),this.fallbackToggle.checked=(h=this.options.enableAutoFallback)!=null?h:!0,this.autoFallbackEnabled=this.fallbackToggle.checked,this.fallbackToggle.style.cssText=`
            ...
            transition: background 0.2s ease;
            outline: none;
        `;let n=()=>{var m;(m=this.fallbackToggle)!=null&&m.checked?(this.fallbackToggle.style.background="var(--ytc-accent)",this.fallbackToggle.style.boxShadow="0 0 10px rgba(45, 212, 191, 0.4)"):(this.fallbackToggle.style.background="var(--ytc-border)",this.fallbackToggle.style.boxShadow="none")},l=a.createDiv();l.style.cssText=`
            position: absolute;
            top: 2px;
            left: 2px;
            width: 12px;
